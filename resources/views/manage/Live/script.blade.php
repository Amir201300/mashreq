...
<script>
    function addFunction()
    {
        ...
        $('#title').text('{{trans("اضافة بث جديد")}}');
...
<script>
          ...
          var id=$('#id').val();
              ...
              url = save_method == 'add' ? "{{route('Live.store')}}" : "{{route('Live.update')}}" ;
          $.ajax({
          url : url,
          type : "post",
          data : formData,
...
<script>
    ...
    function yesDelete()
    {
      ...
      if(checkNum == 1){
      deleteProccess("/manage/Live/delete/" +id_num);
      }else{
        
      deleteProccess("/manage/Live/delete/"+checkArray +'?type=2');
...
<script>
    function ChangeStatus(status,id) {
        TosetV2('{{ trans("main.proccess") }}','info','',false);
        $.ajax({
            url : '/manage/Live/ChangeStatus/' +id +'?status='+status,
            type : 'get',
            success : function(data){
                $.toast().reset('all');
                table.ajax.reload();
